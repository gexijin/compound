...
- **Pacing:** the felt-tension floor rises act over act. Act 4 carries the lightest comedy weight (3); the longest laugh desert is beats 37–40 — the freeze → chain sale → second question → 1987 recognition, the book's gravest run. **Accepted as designed:** the sixth pass deliberately stripped the second-question scene, so the flag stands as a conscious choice, not a defect; if drafting finds Estelle's needle-register carrying dry texture in beat 39, re-score from the draft. The crash (beat 35) satisfies the release rule by cashing a card; the beat-41 hard out releases by cashing cards; beat 46 is the climax. The longest cardless/strandless stretch is beats 17–21 (5, under the flag). The "garage sale was Gary's idea" card is short-held (dealt beat 43 in Friday's contract read, cashed beat 46 at the table) — three beats, but it's a confirmation card (a knife-twist, not a slow burn); accepted, not fixable earlier without contradicting the spine. The former "sell slip" card was removed from the ledger as a category error: Keisha knows the slip is hers, so it's her secret from the *girls* — cast-facing irony, not a fact the reader holds before she does. The vote beat (29) keeps the content, uncarded.
